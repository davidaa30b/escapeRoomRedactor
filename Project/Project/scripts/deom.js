...
function createJsonFile(
  ...
  for (let i = 0; i < hintsCount; i++) {
    const id_hint = i + 1;
    const filename_hint = document.getElementById(`hint${i}-filename`).value;
    const fileext_hint = document.getElementById(`hint${i}-fileext`).value;
    const title_hint = document.getElementById(`hint${i}-title`).value;
    const text_hint = document.getElementById(`hint${i}-text`).value;
...
const oldLockerCount = game.game.setup_game.lockers.length;
const oldHintsCount = game.game.setup_game.story.hints.length;
const oldLockerResources = game.game.setup_game.lockers.map(
  (l) => l.resources.length
);
const oldReflectionQuestionsCount =
  game.game.setup_game.reflection_questions.ol.li.length;
const oldExtensionsCount = game.game.extensions.length;
//////////////////////////////////////////////////////////////////////////////////
...
function generateForm(
  ...
  ///important
  const oldLockerResourcesCopy = lockerResources.slice();
  ///important
  ...
  function generateGeneralInfo() {
    ...
    <option value="Compass">Directions</option>
    </select></label>
  ...
  for (let i = 0; i < lockerCount; i++) {
    ...
    <option value="Digit Lock">Digit Lock</option>
  ...
  for (let i = 0; i < lockerCount; i++) {
    ...
    resultInput.addEventListener("input", function (event) {
      ...
      for (let j = 0; j < resultLength; j++) {
        ...
        fileInput.value =
          i < oldLockerCount ? game.game.setup_game.lockers[i].comment[j] : "";
    ...
    resourcesCountInput.addEventListener("input", function (event) {
      ...
      ///important
      lockerResources[i] = resourcesCount;
      ///important
    });
  ...
  for (let i = 0; i < lockerCount; i++) {
    ...
    const currentLock = document.getElementById(`type_lock_${i}`);
    currentLock.addEventListener("change", function (event) {
      const lockType = event.target.value;
      console.log(event.target.value);

      if (lockType == "Word Lock") {
        document.getElementById(`regex_lock_${i}`).value = word._regex;
        document.getElementById(`result_lock_${i}`).value = word._answer;
        document.getElementById(`logo_lock_${i}`).value = word._logo;
        ...
        for (let j = 0; j < 4; j++) {
          ...
          fileInput.id = `image_input_file_${j}_lock_${i}`;
          fileInput.className = "input_values";
          fileInput.value = word._urls[j];

          lockerInputFileSection.append(fileLabel);
          lockerInputFileSection.append(fileInput);
        }
      } else if (lockType == "Directions Lock") {
        document.getElementById(`regex_lock_${i}`).value = directions._regex;
        document.getElementById(`result_lock_${i}`).value = directions._answer;
        document.getElementById(`logo_lock_${i}`).value = directions._logo;
        ...
        for (let j = 0; j < 4; j++) {
          ...
          fileInput.id = `image_input_file_${j}_lock_${i}`;
          fileInput.className = "input_values";
          fileInput.value = directions._urls[j];

          lockerInputFileSection.append(fileLabel);
          lockerInputFileSection.append(fileInput);
        }
      } else if (lockType == "Digit Lock") {
        document.getElementById(`regex_lock_${i}`).value = digits._regex;
        document.getElementById(`result_lock_${i}`).value = digits._answer;
        document.getElementById(`logo_lock_${i}`).value = digits._logo;
        ...
        for (let j = 0; j < 4; j++) {
          ...
          fileInput.id = `image_input_file_${j}_lock_${i}`;
          fileInput.className = "input_values";
          fileInput.value = digits._urls[j];
  ...
  extContainer.id = "extensions_container";
  ...
  currentLockerCountInput.addEventListener("input", function (event) {
    ...
    lockerResources = new Array(lockerCount);
    console.log("inputConsolelog", lockerCount);
    sectionStoryResources.innerHTML = "";
    ...
    <button id="preview_button_story_resource_${i}">Preview story resource </button>`;
      ...
      const force_order =
        i < oldLockerCount ? game.game.setup_game.lockers[i].force_order : "";
      const lockerResourseCount = i < oldLockerCount ? lockerResources[i] : 0;
    ...
    for (let i = 0; i < lockerCount; i++) {
      ...
      resultInput.addEventListener("input", function (event) {
        ...
        for (let j = 0; j < resultLength; j++) {
          ...
          fileInput.value =
            i < oldLockerCount
              ? game.game.setup_game.lockers[i].comment[j]
              : "";
      ...
      const resourcesSection = document.getElementById(`resources_locker_${i}`);
      let resourcesCount = 0;
      document
        .getElementById(`resource_count_${i}`)
    ...
    for (let i = 0; i < lockerCount; i++) {
      ...
      currentLock.addEventListener("change", function (event) {
        ...
        if (lockType == "Word Lock") {
          ...
          document.getElementById(`regex_lock_${i}`).value = directions._regex;
          document.getElementById(`result_lock_${i}`).value =
            directions._answer;
          document.getElementById(`logo_lock_${i}`).value = directions._logo;
...
////////////////////////////////////////////////////////////////////////////////////////////
class Locker {
  constructor(regex1, answer, urlsNum, logo, url1, url2, url3, url4) {
    this._regex = regex1;
    this._logo = logo;
    this._answer = answer;
    this._numberOfUrls = urlsNum;
    this._urls = [url1, url2, url3, url4];
  }
...
const digits = new Locker(
  "[0-9]",
  "1234",
  4,
  "https://cdn11.bigcommerce.com/s-m28of/images/stencil/original/products/1473/4288/4288__55344.1690306225.png?c=2",
  ...
  "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fd/Eo_circle_pink_number-4.svg/2048px-Eo_circle_pink_number-4.svg.png"
);

const directions = new Locker(
  "(w|s|e|n|ne|nw|se|sw)",
  "ewns",
  4,
  "https://upload.wikimedia.org/wikipedia/commons/thumb/9/99/Compass_rose_simple.svg/1200px-Compass_rose_simple.svg.png",
  "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT_OiuvliFOVyVNXE9al05rl1eRW1gJTsZrJA&s",
  "https://bitprice.ru/sites/default/files/img/brands/logo/446692.png",
  "https://cdn3.vectorstock.com/i/1000x1000/68/82/north-arrow-black-high-quality-vector-42946882.jpg",
  "https://ecdn.teacherspayteachers.com/thumbitem/Compass-Directions-North-East-South-West-ZisforZebra-Freebie-076525300-1379636202-1397349079/original-881027-3.jpg"
);

const word = new Locker(
  "[a-z]",
  "abcd",
  4,
  "https://cdn.teachercreated.com/20240118/covers/900sqp/9123.png",
  "https://cdn3.iconfinder.com/data/icons/block/32/letter-512.png",
  "https://i.pinimg.com/originals/b0/af/c1/b0afc11b539c6867e89d6765bbfb65c4.png",
  "https://i.pinimg.com/736x/96/5a/ec/965aecc908fb8b7c6b84421e6dd1cfa7.jpg",
