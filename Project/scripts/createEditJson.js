...
function generateSections(inputLockerCount) {
  ...
  for (let refQ = 0; refQ < curReflectionQuestionsCount; refQ++) {
    ...
    refQuestions += `<section> <input id="ref-quest${refQ}" value="${ref_quest}" type="text"> </section>`;
  }
    ...
    for (let refQ = 0; refQ < curReflectionQuestionsCount; refQ++) {
      const ref_quest =
        refQ < reflectionQuestionsCount
          ...
          : "";
      part += `<section><input id="ref-quest${refQ}" value="${ref_quest}" type="text"></section>`;
    }
  ...
  sectionForButton.innerHTML =
    '<button id="create-json"> Download json </button>';
