...
window.addEventListener("load", function () {
  ...
  gameSection.id = "game_json_form";
  gameSection.innerHTML = "";
  this.document.body.appendChild(gameSection);
...
function generateForm(lockerCount, lockerResources, hintsCount, refQuesCount) {
  ...
  currentLockerCountInput.addEventListener("input", function (event) {
    ...
    gameSection.innerHTML = "";
    console.log(lockerCount, lockerResources, hintsCount, refQuesCount);
    generateForm(lockerCount, lockerResources, hintsCount, refQuesCount);
  });

  const outputSection = document.createElement("section");
  outputSection.innerHTML = ``;
  gameSection.appendChild(outputSection);

  const errorSection = document.createElement("section");
  errorSection.innerHTML = "";
  gameSection.appendChild(errorSection);
  ...
  sectionForDownloadOrSaveJsonButton.innerHTML = `<button id="download-json" class="button-download"> Download json </button>
     <button id="save-json" class="hidden-button button-action"> Save json </button>
     <button id="update-json" class="hidden-button button-action"> Update json </button>
     `;
  gameSection.appendChild(sectionForDownloadOrSaveJsonButton);
  ...
  if (gameContent === "empty") {
    document.getElementById("save-json").classList.remove("hidden-button");
    document.getElementById("save-json").addEventListener("click", (event) => {
      saveGameJson();
      event.preventDefault();
    });
  } else {
    document.getElementById("update-json").classList.remove("hidden-button");
    document
  ...
  gameSection.appendChild(footerSection);
  ...
  function downloadJsonFile() {
    ...
    const jsonString = JSON.stringify(gameNew);
    ...
    return fetch("../controllers/saveGameJsonController.php", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          outputSection.innerHTML = `<section class="success-message-action">The ${game.game.name} was uploaded successfully.</section>`;
    ...
    return fetch("../controllers/updateGameJsonController.php", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          outputSection.innerHTML = `<section class="success-message-action">The ${game.game.name} was updated successfully.</section>`;
