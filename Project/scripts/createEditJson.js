...
function generateForm(
  ...
  async function generateLockersAndStoryResources() {
    ...
    const options = [];
    const readyLock = await fetchLockers();
    ...
    for (let i = 0; i < lockerCount; i++) {
      ...
      readyLock.forEach((type) => {
        typeOptions += `<option value="${type.id}">${type.type}</option>`;
      });
    ...
    for (let i = 0; i < lockerCount; i++) {
      ...
      currentLock.addEventListener("change", function (event) {
        ...
        console.log(readyLock[lockType - 1]);

        const lockShow = readyLock[lockType - 1];
        const lockerInputFileSection = document.getElementById(
          `locker_input_files_${i}`
        );
        ...
        if (lockShow !== undefined) {
          document.getElementById(`regex_lock_${i}`).value = lockShow.regex;
          document.getElementById(`result_lock_${i}`).value = lockShow.answer;
          document.getElementById(`logo_lock_${i}`).value = lockShow.logo;
          document.getElementById(`number_attempts_lock_${i}`).value =
            lockShow.attemptsNumber;
          document.getElementById(`show_attempts_lock_${i}`).value =
            lockShow.showAttempts;
          document.getElementById(`points_lock_${i}`).value = lockShow.points;
          document.getElementById(`force_order_lock_${i}`).value =
            lockShow.forceOrder;
          document.getElementById(`resource_count_${i}`).value =
            lockShow.resourcesNum;

          resourcesSection.innerHTML = "";
          lockerInputFileSection.innerHTML = "";
          console.log(lockShow.urlsNum);
          for (let j = 0; j < lockShow.urlsNum; j++) {
            const fileLabel = document.createElement("label");
            fileLabel.htmlFor = `image_input_file_${j}_lock_${i}`;
            fileLabel.textContent = `File url ${j + 1}:`;
            ...
            fileInput.id = `image_input_file_${j}_lock_${i}`;
            fileInput.className = "input_values";
            fileInput.value = JSON.parse(lockShow.usrls)[j];

            lockerInputFileSection.append(fileLabel);
            lockerInputFileSection.append(fileInput);
          }
          for (let j = 0; j < lockShow.resourcesNum; j++) {
            const resourceInputLabel = document.createElement("label");
            resourceInputLabel.htmlFor = `resources_${j}_locker_${i}`;
            resourceInputLabel.textContent = `Resource ${j + 1}:`;
            ...
            resourceInput.type = "text";
            resourceInput.id = `resources_${j}_locker_${i}`;
            resourceInput.value = JSON.parse(lockShow.resources)[j].link;
            resourcesSection.append(resourceInputLabel);
            resourcesSection.append(resourceInput);
          }
        } else {
          ...
          resourcesSection.innerHTML = "";
          lockerInputFileSection.innerHTML = "";
          document.getElementById(`regex_lock_${i}`).value = "";
          document.getElementById(`result_lock_${i}`).value = "";
          document.getElementById(`logo_lock_${i}`).value = "";
          document.getElementById(`number_attempts_lock_${i}`).value = 0;
          document.getElementById(`show_attempts_lock_${i}`).value = "";
          document.getElementById(`points_lock_${i}`).value = 0;
          document.getElementById(`force_order_lock_${i}`).value = "";
          document.getElementById(`resource_count_${i}`).value = 0;
        }
      });
  ...
  generateLockersAndStoryResources();
...
async function fetchLockers() {
  try {
    const response = await fetch("../controllers/fetchLockersController.php");
    const data = await response.json();
    return data;
  } catch (error) {
    console.log("Error fetching lock types:", error);
    return [];
  }
}
